...
Undo 時限結束後，手機會把不可覆寫的刪除墓碑同步到 Google 中繼站；平台尚未匯入的對應採集資料會改為 `discarded`，避免已刪除資料重新出現。若資料已進正式帳本，手機刪除不會回溯刪除正式紀錄。
...
目前保留 IndexedDB 作為主要離線資料庫，並已透過 Google Apps Script 與 Google Sheet 提供採集事件中繼。交易永遠先存手機；有網路時才上傳，中繼站不是真正帳本。

未來仍可把中繼實作替換為 Google Drive JSON、Google Sheets API 或自有後端，而不改變交易 ID、版本與收據模型。
...
公開版沒有寫死 Google API key、OAuth token 或配對碼。Apps Script 網址與配對碼由使用者在手機本機設定。

## 目前限制

- 不會直接連線桌面平台 API。
- 設定 Google 中繼後會自動嘗試上傳；未設定時仍可完全離線使用及手動匯出。
...
- 頁首版本小字可用來確認目前載入的測試版本。
- 平台可經 Google 中繼發布分類、帳戶與付款方式唯讀快照；手機只能選用，不能修改平台正式設定。
- 詳細同步階段為 `local_saved`、`sync_pending`、`synced_to_google`、`ready_for_import`、`imported`、`failed`、`conflict`。
